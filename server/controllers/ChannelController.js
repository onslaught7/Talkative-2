...
export const createChannel = async (request, response, next) => {
    try {
        const { name, members, admins } = request.body; // Accept multiple admins
        const userId = request.userId;

        const validMembers = await User.find({ _id: { $in: members }});

        if (validMembers.length !== members.length) {
            return response.status(400).send("Some users are not valid members.");
        }

        // Ensure all provided admins are also valid members
        if (!admins.every(adminId => members.includes(adminId))) {
        ...
        const newChannel = new Channel({
            name, 
            members,
            admins: admins || [userId], // Default to creator as admin if none are provided
        });
